...
use constant MAINNET => {
    ...
    ADDRESS_VER        => "\x80",
    DELEG_KEY_VER      => "\x8d", # delegation WIF: privkey + hash256(delegate pubkey)
    ADDR_MAGIC         => "\x13\x9d",
    PKH_MAGIC          => "\x27\x10", # base58 pubkeyhash strings start with "6n"
...
use constant TESTNET => {
    ...
    QBT_BURN_HASH      => pack("H*", "fe5205472fb87124923f4be64292ef289478b06d"), # 1QBitcoin1QBitcoin1QBitcoin1pSAg3e
    ADDRESS_VER        => "\xef",
    DELEG_KEY_VER      => "\xf0", # delegation WIF: privkey + hash256(delegate pubkey)
    ADDR_MAGIC         => "\x04\x73\x89",
    PKH_MAGIC          => "\x3f\x00", # base58 pubkeyhash strings start with "AKX"
