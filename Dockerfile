...
# docker run --volume $(pwd)/database:/database --read-only --rm --detach -p 9666:9666 -p 127.0.0.1:9669:9669 --name qecurrency qecurrency
# or:
# docker run -e dbi=mysql --mount type=bind,source=/etc/qecurrency.conf,target=/etc/qecurrency.conf,readonly -mount type=bind,source=/var/run/mysqld/mysqld.sock,target=/var/lib/mysql.sock --rm --detach -p 9666:9666 -p 127.0.0.1:9669:9669 --name qecurrency qecurrency
...
RUN cd /qecurrency; rm -rf test systemd Dockerfile Makefile admin/Makefile admin/src admin/node_modules
